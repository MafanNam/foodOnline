...
def place_order(request):
    ...
    k = {}
    for i in cart_items:
        food_item = FoodItem.objects.get(pk=i.food_item.id, vendor_id__in=vendors_ids)
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        for item in cart_items:
            ...
            ordered_food.amount = item.food_item.price * item.quantity  # total amount
            ordered_food.save()
        ...
        for i in cart_items:
            if i.food_item.vendor.user.email not in to_emails:
                ...
                ordered_food_to_vendor = OrderedFood.objects.filter(order=order, food_item__vendor=i.food_item.vendor)
